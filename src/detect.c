        ...
        Packet *p)
{
    ...
    /* 不需要检测, 直接退出检测流程; bail early if packet should not be inspected */
    ...
    /* 遍历prefilter/non-prefilter已匹配的规则，逐个精确匹配；并运行报文检测引擎 */
        ...
        const Packet *p)
{
    ...
    if (p->proto == 0 && p->events.cnt > 0) {       /* 无L4协议，则仅检测其解析事件规则组 */
...
{   /* 流和非流的区别: 流会保存已检测标识, 避免相同流的后续报文继续逐包检测 */
    if (pflow) {
            ...
            ((p->flowflags & FLOW_PKT_TOCLIENT) && !(p->flowflags & FLOW_PKT_TOCLIENT_IPONLY_SET)))
        {
            ...
            /* 设置已检测标识, save in the flow that we scanned this direction... */
...
{
    /* 流变量匹配，check if this signature has a requirement for flowvars of some type
    ...
    /* 匹配源、目的IP，check the destination address */
...
{   /* 根据报文属性，获取非多模规则集（非prefilter） */
    ...
    /* 合并已匹配的pf、non-pf规则为链表，create match list if we have non-pf and/or pf */
...
)
{
...
#endif
    /* 遍历已匹配的规则列表, DetectEngineThreadCtx->match_array[] */
    ...
    while (match_cnt--) {
        ...
        if (DetectEnginePktInspectionRun(tv, det_ctx, s, pflow, p, &alert_flags) == false) {
            goto next;         /* 基于报文引擎的匹配 Signature->pkt_inspect */
        }
    ...
    Packet * const p, Flow * const pflow)
{
    ...
    if (p->flags & PKT_HAS_FLOW) {     /* 读取流相关信息 */
        ...
        if (pflow->de_ctx_version == 0) {  /* 记录引擎版本号，以发现引擎是否重加载 */
            ...
            pflow->flowvar = NULL;         /* 重新加载了检测引擎, 释放流上关联信息 */

            DetectEngineStateResetTxs(pflow);    /* 重置流上事务相关信息 */
        }
...
static inline void DetectRunPostRules(
    ...
    DetectRunScratchpad *scratch)
{
    /* 事务相关操作，see if we need to increment the inspect_id and reset the de_state */
        ...
        DetectRunScratchpad *scratch)
{
    ...
    /* 对于新开始的检测, 检测报文内容; for a new inspection we inspect pkt header and packet matches */
    ...
    while (engine != NULL) { // TODO could be do {} while as s->app_inspect cannot be null
        ...
        if (!(inspect_flags & BIT_U32(engine->id)) && /* 不重复检测 */
                direction == engine->dir)
        {
            ...
            if (unlikely(skip_engine)) {              /* 不匹配的应用协议ID */
                engine = engine->next;
                continue;
            }
            ...
            if (tx->tx_progress < engine->progress) { /* 不匹配的处理阶段 */
            ...
            if (unlikely(engine->stream && can->stream_stored)) {
                ...
                if (engine->Callback) {               /* 检测回调;  http.uri ==> DetectEngineInspectBufferGeneric() */
                    match = engine->Callback(tv, de_ctx, det_ctx,/* file_data ==> DetectEngineInspectFiledata() */
            ...
            if (match == DETECT_ENGINE_INSPECT_SIG_MATCH) {
                ...
                engine = engine->next;                /* 检测匹配后, 记录, 避免后续继续匹配 */
                total_matches++;
                continue;
                ...
                engine = engine->next;                /* 事务中包含多文件的情形, 后续仍然需要继续匹配 */
                total_matches++;
                continue;
                    ...
                    DetectRunScratchpad *scratch)
{
    ...
    while (1) {
        ...
        /* 加入(非事务)已经匹配的规则, merge 'state' rules from the regular prefilter */
        ...
        /* 加入保留的规则, merge stored state into results */
        if (tx.de_state != NULL) {
        ...
        /* 逐个匹配候选规则, run rules: inspect the match candidates */
        for (uint32_t i = 0; i < array_idx; i++) {
            ...
            if (inspect_flags) {          /* 已经运行过检测, 无需再次匹配 */
...
{
    PACKET_UPDATE_ACTION(p, s->action);   /* 更新报文动作标识 */

    if (s->action & ACTION_DROP) {        /* 记录DROP动作 */
        if (p->alerts.drop.action == 0) {
            p->alerts.drop.num = s->num;
            p->alerts.drop.action = s->action;
            p->alerts.drop.s = (Signature *)s;
        }
    } else if (s->action & ACTION_PASS) { /* 打标签，后续此流的报文不再规则检测 */
...
TmEcode Detect(ThreadVars *tv, Packet *p, void *data)
{
    ...
    if (p->flow) {
        DetectFlow(tv, de_ctx, det_ctx, p);    /* 流检测 */
    } else {
        DetectNoFlow(tv, de_ctx, det_ctx, p);  /* 非流检测 */
    }
    return TM_ECODE_OK;
error:
