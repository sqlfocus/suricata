...
typedef struct AppLayerProtoDetectProbingParserElement_ {
    ...
    uint32_t min_depth;                  /* ALPROTO_TLS -> 0 */
    /* the max length of data after which this parser won't be invoked */
    uint32_t max_depth;                  /* ALPROTO_TLS -> 3 */

    /* the to_server probing parser function */
    ProbingParserFPtr ProbingParserTs;   /* ALPROTO_TLS -> SSLProbingParser() */
...
typedef struct AppLayerProtoDetectProbingParserPort_ {
    ...
    uint16_t dp_max_depth;   /* ->dp 最大匹配深度; to server */
    uint16_t sp_max_depth;   /* ->sp 最大匹配深度; to client */
    /* <TK!!!>每个端口只能对应某个协议, 不能对应多个 */
    AppLayerProtoDetectProbingParserElement *dp;    /* 匹配应用协议列表; 特定端口的最后都 */
...
} AppLayerProtoDetectProbingParser;    /* 知名端口列表: 以传输层协议组织端口, alpd_ctx->ctx_pp[] */
...
typedef struct AppLayerProtoDetectPMSignature_ {
    ...
    uint16_t pp_min_depth;  /* ->cd对应的最小检测长度 */
    uint16_t pp_max_depth;  /* ->cd对应的最大检测长度 */
    ProbingParserFPtr PPFunc;  /* 基于port识别时, 用于进一步内容检测确认 */
...
typedef struct AppLayerProtoDetectPMCtx_ {
    uint16_t pp_max_len;    /* 检测深度的最大值 */
    uint16_t min_len;       /*           最小值 */
     ...
     *  the pattern id. */
    AppLayerProtoDetectPMSignature **map;   /* 指针数组, 存放->head链表对象地址 */
    AppLayerProtoDetectPMSignature *head;   /* 注册的匹配模式, 初始化后为NULL */
...
typedef struct AppLayerProtoDetectCtx_ {
    ...
    AppLayerProtoDetectCtxIpproto ctx_ipp[FLOW_PROTO_DEFAULT];
                                                 /* 基于规则匹配的多模环境: 传输层协议->流方向->L7识别规则表  */
    /* Global SPM thread context prototype. */
    SpmGlobalThreadCtx *spm_global_thread_ctx;   /* 基于规则匹配的单模环境; 多模初筛过后, 逐规则匹配 */

    AppLayerProtoDetectProbingParser *ctx_pp;    /* ”协议<-->端口号"的对应关系列表，如ssl <--> 443 */

    /* Indicates the protocols that have registered themselves
     * for protocol detection.  This table is independent of the
     * ipproto. */
    const char *alproto_names[ALPROTO_MAX];      /* 待检测的协议名称，如"http" */
...
struct AppLayerProtoDetectThreadCtx_ {
    ...
    MpmThreadCtx mpm_tctx[FLOW_PROTO_DEFAULT][2];  /* 多模引擎线程环境 */
    SpmThreadCtx *spm_thread_ctx;                  /* 单模引擎线程环境 */
};

/* 应用层协议检测相关的环境，The global app layer proto detection context. */
        ...
        bool *rflow)
{
    ...
    uint8_t *found = SpmScan(s->cd->spm_ctx, tctx->spm_thread_ctx,
            sbuf, ssearchlen);  /* 单模引擎匹配, DetectContentData->spm_ctx */
    if (found == NULL) {        /* 未匹配 */
        SCReturnUInt(ALPROTO_UNKNOWN);
    }
    ...
    SCReturnUInt(s->alproto);   /* 返回规则对应的应用协议 */
}
        ...
        uint8_t direction, AppProto *pm_results, bool *rflow)
{
    ...
    for (uint32_t cnt = 0; cnt < tctx->pmq.rule_id_array_cnt; cnt++) {
        ...
        while (s != NULL) {  /* 遍历已匹配的规则，继续单模匹配，得到应用协议: 因为注册时, 有的pattern设定了offset和depth,  */
            AppProto proto = AppLayerProtoDetectPMMatchSignature(s, /* 而多模匹配时, 忽略了这些细节 */
    ...
    PmqReset(&tctx->pmq);
    return pm_matches;       /* 返回匹配的应用数，结果存放在参数pm_results[]中 */
}
        ...
        uint8_t direction, AppProto *pm_results, bool *rflow)
{
    ...
    /* pattern found, yay */
    if (m > 0) {
        FLOW_SET_PM_DONE(f, direction);     /* 标识已完成协议识别 */
    ...
    } else if (m <= 0) {              /* CASE: 中间建流且正方向未匹配; 抽取反方向规则引擎, 继续匹配 */
                                                             ...
                                                             ProbingParserFPtr ProbingParser2)
{
    ...
    if (curr_pp == NULL) {
        ...
        new_pp->ipproto = ipproto; /* 注册特定协议, 挂接到链表尾端 */
    ...
    if (curr_port == NULL) {
        ...
        new_port->port = port;     /* 注册特定端口, 挂接到链表尾端 */
    ...
    if (direction & STREAM_TOSERVER)       /* 根据方向, 选择插入端口列表 */
...
static int AppLayerProtoDetectPMSetContentIDs(AppLayerProtoDetectPMCtx *ctx)
{
    ...
    typedef struct TempContainer_ {
        ...
        uint8_t *content;   /* 类似于 content[0]，分配内存时多分配对应长度 */
    } TempContainer;
    ...
    for (s = ctx->head; s != NULL; s = s->next) {
        ...
        ctx->max_sig_id++;    /* 遍历已注册的单模规则列表 AppLayerProtoDetectPMCtx->head */
    }

    ahb = SCMalloc(sizeof(uint8_t) * (struct_total_size + content_total_size));
    if (unlikely(ahb == NULL))/* 分配内存，TempContainer + content */
        goto error;
    ...
    for (s = ctx->head; s != NULL; s = s->next) {
        ...
        s->cd->id = struct_offset->id;  /* 更新 DetectContentData->id */

        struct_offset++;
    }

    ctx->max_pat_id = max_id;    /* 记录规则最大ID */
...
static int AppLayerProtoDetectPMMapSignatures(AppLayerProtoDetectPMCtx *ctx)
{
    ...
    for (s = ctx->head; s != NULL; ) {
        ...
        ctx->map[s->id] = s;  /* 存储signature到->map[]指针数组 */
        s->next = NULL;
        s = next_s;
    }
    ctx->head = NULL;         /* 设置链表为NULL，->map查询速度更快 */
                              /* 注意逐步初始化过程中->head更便于存储可变数量的对象 */
    goto end;
 error:
    ret = -1;
 end:
...
static int AppLayerProtoDetectPMPrepareMpm(AppLayerProtoDetectPMCtx *ctx)
{
    ...
    if (mpm_table[mpm_ctx->mpm_type].Prepare(mpm_ctx) < 0)
        goto error;    /* 构建多模匹配引擎, MPM_HS -> SCHSPreparePatterns() */
                                                ...
                                                uint16_t pp_min_depth, uint16_t pp_max_depth)
{
    ...
    AppLayerProtoDetectPMAddSignature(ctx_pm, cd, alproto, direction,
            PPFunc, pp_min_depth, pp_max_depth);
                               /* 加入链表 alpd_ctx.ctx_ipp.ctx_pm.head */
    goto end;
 error:
...
AppProto AppLayerProtoDetectGetProto(AppLayerProtoDetectThreadCtx *tctx,
                                     ...
                                     bool *reverse_flow)
{/* 此函数依赖的检测环境在函数 AppLayerSetup() 初始化 */
    SCEnter();
    ...
    if (!FLOW_IS_PM_DONE(f, direction)) {  /* CASE1: 基于关键字的应用识别 */
        ...
        if (pm_matches > 0) {
            alproto = pm_results[0];       /* 多条匹配时，仅取第一条结果 */
                                           /* 因此匹配规则应该按优先级排序 */
            ...
            if (!(ipproto == IPPROTO_UDP && alproto == ALPROTO_DCERPC))
                goto end;                  /* 除 ALPROTO_DCERPC 外，不需要后续基于端口的识别 */
    ...
    if (!FLOW_IS_PE_DONE(f, direction)) {  /* CASE3: 基于特殊设定的协议识别, 如ftp的父子通道关联（父通道决定子通道应用协议为FTP） */
        alproto = AppLayerProtoDetectPEGetProto(f, ipproto, direction);
    }

 end:
    if (!AppProtoIsValid(alproto))
        alproto = pm_alproto;              /* 返回识别协议 */
                                   ...
                                   ProbingParserFPtr ProbingParser2)
{
    ...
    DetectPortParse(NULL,&head, portstr);    /* 解析端口配置，如"80"/"80,8080" */
    DetectPort *temp_dp = head;
    while (temp_dp != NULL) {
        uint32_t port = temp_dp->port;
        if (port == 0 && temp_dp->port2 != 0)
            port++;                          /* 将端口加入 alpd_ctx.ctx_pp[]；0除外 */
                                         ...
                                         ProbingParserFPtr ProbingParserTc)
{
    ...
    if (port_node != NULL && port_node->val != NULL) {
        AppLayerProtoDetectPPRegister(ipproto,    /* 注册到 alpd_ctx->ctx_pp[] */
                                      port_node->val,
                                      alproto,
                                      min_depth, max_depth,
...
int AppLayerProtoDetectSetup(void)
{
    ...
    uint16_t spm_matcher = SinglePatternMatchDefaultMatcher(); /* SPM_HS */
    uint16_t mpm_matcher = PatternMatchDefaultMatcher();       /* MPM_HS */
...
AppLayerProtoDetectThreadCtx *AppLayerProtoDetectGetCtxThread(void)
{
    ...
    /* 存放Get the max pat id for all the mpm ctxs. */
    if (PmqSetup(&alpd_tctx->pmq) < 0)
        goto error;
    /* 初始化线程多模识别环境，复制scratch空间 */
    for (i = 0; i < FLOW_PROTO_DEFAULT; i++) {
        for (j = 0; j < 2; j++) {
